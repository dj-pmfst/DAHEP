...
theta = np.linspace(0,12,100)
xi = [xi2(i) for i in theta]
...
chi2_shifted = xi - (chi2_min + 1)

left = interp1d(chi2_shifted[theta < theta_hat], theta[theta < theta_hat])
theta_minus = left(0)

right = interp1d(chi2_shifted[theta > theta_hat], theta[theta > theta_hat])
theta_plus = right(0)
...
plt.axhline(chi2_min, linestyle='--', color='red')
plt.axhline(chi2_min + 1, linestyle='--', color='red')
